...
export function render(token: string): string {
  const verifyLink = `${emailConfig.baseUrl}/verify-email/${token}`;
...
<html lang="en">
  <head>
    ...
    <title>Verify your EchoLog email</title>
  </head>
      ...
      <tr>
        <td style="padding: 0 40px 12px;">
          ...
          <p style="margin: 0 0 16px; font-size: 16px; line-height: 1.6; color: #374151;">
            Click the button below to verify your email address and unlock the ability to create multiple workspaces on EchoLog.
          </p>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td style="padding: 4px 40px 12px;">
          <p style="margin: 0; font-size: 13px; color: #9ca3af;">
            Or copy this link: <a href="${verifyLink}" style="color: #4f46e5; word-break: break-all;">${verifyLink}</a>
          </p>
        </td>
      </tr>
      <tr>
        <td style="padding: 20px 40px;">
          <p style="margin: 0; font-size: 14px; color: #6b7280; line-height: 1.5;">
            This link expires in 24 hours. If you did not create an EchoLog account, you can safely ignore this email.
          </p>
        </td>
      </tr>
      <tr>
