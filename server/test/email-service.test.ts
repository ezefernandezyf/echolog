...
describe('EmailService', () => {
  ...
  describe('sendVerificationEmail', () => {
    ...
    it('calls resend.emails.send with correct params', async () => {
      ...
      await service.sendVerificationEmail('tok-verify-123', 'user@test.com');

      expect(mockSend).toHaveBeenCalledTimes(1);
      const callArgs = mockSend.mock.calls[0][0];
      expect(callArgs.to).toBe('user@test.com');
      expect(callArgs.subject).toBe('Verify your EchoLog email');
      expect(callArgs.html).toContain('tok-verify-123');
      expect(callArgs.html).toContain('/verify-email/');
      expect(callArgs.html).toContain('24 hours');
    });

    it('returns immediately when NODE_ENV=test', async () => {
      process.env.NODE_ENV = 'test';

      await service.sendVerificationEmail('tok', 'user@test.com');
    ...
    it('logs error and does not throw when Resend fails', async () => {
      ...
      await service.sendVerificationEmail('tok', 'fail@test.com');
